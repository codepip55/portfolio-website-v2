...
try {
  ...
  const packageLockJsonData = JSON.parse(fs.readFileSync(packageLockJsonPath, 'utf8'));
  ...
  fs.writeFileSync(packageLockJsonPath, JSON.stringify(packageLockJsonData, null, 2));

  console.log(`Version updated to ${newVersion} in package.json and package-lock.json.`);
} catch (err) {
